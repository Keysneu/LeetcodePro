      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <div className="lc-card p-4">
          ...
          {loading || !heatmapOption ? (
            <div className="h-[280px] animate-pulse rounded-lg bg-[var(--lc-surface-soft)]" />
          ) : (
            <ReactECharts option={heatmapOption} style={{ height: 320, width: "100%" }} notMerge lazyUpdate />
          )}
        </div>
        ...
        <div className="lc-card p-4">
          ...
          {loading ? (
            <div className="h-[320px] animate-pulse rounded-lg bg-[var(--lc-surface-soft)]" />
          ) : !radarOption ? (
            <div className="flex h-[320px] items-center justify-center rounded-lg border border-dashed text-sm text-[var(--lc-text-muted)]">
              当前暂无可展示的标签数据
            </div>
          ) : (
            <ReactECharts option={radarOption} style={{ height: 340, width: "100%" }} notMerge lazyUpdate />
          )}
        </div>
      </div>
    </section>
