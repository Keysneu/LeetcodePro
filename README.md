...
15. 在右侧编辑器保持默认代码或粘贴可 AC 代码，点击 `提交判题`。
16. 观察“判题结果”面板状态从 `QUEUED/RUNNING` 变为终态（如 `AC`）。
13. 在左侧切到 `AI判题` 页签，将模型从 `vLLM（远程）` 切到 `MiniMax（远程）`。
14. 点击 `AI判题`，先观察结果区顶部出现“当前阶段”提示（例如“正在检索失败样例、用户笔记与结构化证据”），并带有等待时长。
15. 若本次未 AC，确认 `AI判题` 返回“主要问题 + 具体修改建议 + 快速验证”；若已 AC，应返回“通过后优化评审”。
16. 若失败题目存在公开失败样例，确认 AI 文本会引用“输入 / 你的输出 / 期望输出”中的具体值，而不是只返回泛化模板。
17. 在左侧切到 `AI题解` 页签，将模型切换为 `vLLM（远程）` 后点击 `生成题解`。
18. 观察 `AI题解` 区域同样先显示“当前阶段”，随后再开始流式正文。
19. 在 `AI题解` 或 `AI判题` 的 `thinking` 面板点击 `收起 thinking`，确认思考区可折叠；再次点击 `展开 thinking` 后，先前已流出的内容仍然保留。
20. 在 `thinking` 面板点击 `放大 thinking`，确认思考区高度明显增加且内部可滚动；再点击 `标准视图` 可恢复常规高度。
20. 让模型输出包含 Markdown 代码块或列表的长回答，确认正文会边流边渲染，而不是等到整个响应结束后才一次性排版完成。
21. 点击顶部导航右上角主题按钮，在“夜间/白天”之间切换 2 次，确认页面背景、卡片、文字和 Monaco 编辑器主题同步切换。
22. 在题目左侧确认存在 5 个页签：`描述 / 提交记录 / 笔记题解 / AI题解 / AI判题`。
23. 切换到 `提交记录/笔记题解/AI题解/AI判题`，确认都不显示题目标题与徽标；仅 `描述` 显示标题与徽标。
24. 在 `提交记录` 点击任意历史提交，确认站内弹窗出现；先点“取消”，编辑器与右侧判题结果保持不变。
25. 再次点击同一条历史提交并确认，验证编辑器代码回填、右侧判题结果同步为该次提交，且左侧页签不自动跳转。
26. 切到 `AI判题`，点击“AI判题”，确认返回 Markdown 诊断文本（会结合提交状态、失败样例与错误信息）。
27. 切到 `AI题解`，点击“生成题解”，确认可正常生成并与当前回放 `submissionId` 绑定。
28. 在 `AI题解` 与 `AI判题` 观察结果区，确认不再显示 `模型/来源/服务返回模型/会话/当前提交` 这些调试信息。
29. 返回首页 `http://localhost:3000/`，在“上传刷题笔记（全局）”卡片上传包含多题笔记的 Markdown。
30. 回到题目页 `笔记题解` 标签，确认“我的题解笔记”区域能展示当前题匹配内容。
31. 将浏览器拉宽到 `1920px` 或 `2560px`，确认 `题库/进度/后台` 页面主体区域随窗口扩展，不再被 `1400px` 限制。
32. 在 `http://localhost:3000/problems/two-sum` 桌面端确认左右默认分栏约 `40/60`，拖拽中间分隔条可实时调整宽度；双击分隔条恢复默认比例。
33. 刷新页面确认分栏比例保持；将窗口缩到平板/手机宽度后确认自动切换为上下堆叠，且拖拽分隔条不显示。
34. 在题目页右侧确认“代码框/运行与分析”默认约 `58/42`，拖拽中间横向分隔条后高度实时变化，双击分隔条恢复默认比例。
35. 观察右侧“判题结果”核心指标，确认以横向紧凑统计条展示（状态/运行时间/内存/通过数），空间不足自动换行。
36. 将窗口缩到平板/手机宽度，确认纵向分栏自动回退为普通上下堆叠且不显示拖拽条。
37. 在题目页左侧来回切换 `描述/提交记录/笔记题解/AI题解/AI判题`，确认左右面板整体高度保持一致，不出现跳变。
38. 在右侧“运行与分析”点击“判题结果”卡片的“收起/展开”，确认状态可切换且刷新后保持。
39. 在 `AI题解` 与 `AI判题` 分别切换不同模型（例如一个选 `vLLM`、一个选 `MiniMax`），刷新后确认两者各自保持，不互相覆盖。
40. 在桌面端把浏览器高度压缩到约 `700~850px`，确认右侧代码编辑器主体仍可见且可输入，不会被顶部挤出可视区域。
41. 在 `core/acm` 模式间来回切换 3 次，并切换 `C++/Python` 各 1 次，确认顶部“模式/语言”下拉与编辑区首行始终完整可见，不会出现被顶出或只剩一条细线。
